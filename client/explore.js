...
//   let user = localStorage.getItem("user_login_name") || "Guest";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function showFiltered(all) {
    const val = filter.value;
    const filtered = val === "all" ? all : all.filter((i) => i.category === val);
  ...
  function render(data) {
    ...
    const currentUserId = userObj ? (userObj.id || userObj._id) : null;
    ...
    data.forEach((item) => {
      ...
      if (item.comments && item.comments.length > 0) {
        item.comments.forEach((c) => {
          const authorName = c.author?.username || c.username || "Anonymous";
          cmtsHTML += `<div style="margin-bottom:8px; border-bottom:1px dashed #f1f5f9; padding-bottom:4px;"><strong>${authorName}:</strong> <span>${c.text}</span></div>`;
        });
      }
      ...
      const isLiked = currentUserId ? likesArray.some(id => (id._id || id) === currentUserId) : false;


      let tagsHTML = "";
      if (item.tags && item.tags.length > 0) {
        item.tags.forEach(t => {
          tagsHTML += `<span style="background: #f1f5f9; color: #475569; font-size: 11px; padding: 2px 8px; border-radius: 4px; margin-right: 5px; font-weight: 5px;">${t}</span>`;
        });
      }

      const currentLoggedInUser = localStorage.getItem("user_login_name");
      const isAuthor = item.author && (item.author.username === currentLoggedInUser || item.author === currentLoggedInUser || item.author?._id === currentUserId);
      ...
      card.innerHTML = `
                <div class="view-block">
                    ...
                    <div class="idea-tags" style="margin-bottom: 10px; display: flex; flex-wrap: wrap; gap: 4px;">
                        ${tagsHTML}
                    </div>

                    <div class="card-meta">By <strong>${item.author?.username || "Creator"}</strong></div>
                    <div class="card-stats" style="margin-top:12px; user-select:none;">
                        <span class="like-btn" data-id="${item._id}" style="cursor:pointer; margin-right:15px;">
                            <i class="${isLiked ? "fa-solid fa-heart" : "fa-regular fa-heart"}" style="color:${isLiked ? "#dc2626" : "#64748b"};"></i> <span class="like-count">${likesCount}</span>
                        </span>
                        <span class="cmt-btn" data-id="${item._id}" style="cursor:pointer;">
                            <i class="fa-regular fa-comment"></i> <span class="cmt-count">${item.comments ? item.comments.length : 0}</span>
                        </span>
  ...
  function bindActions() {
    document.querySelectorAll(".like-btn").forEach((b) => {
      b.onclick = async function () {
        ...
        try {
          ...
          const res = await fetch(`${URL}/ideas/${id}/like`, {
            method: "PUT", 
            headers: getRequestHeaders({"Content-Type": "application/json"}),
            credentials: "include",
          });
          ...
          if(res.ok) {
             ...
             await fetch(`${URL}/ideas/${id}/like`, { method: "POST", headers: getRequestHeaders(), credentials: "include" });
             getIdeas();
          }
        } catch (err) {
            console.error("Like execution error:", err);
        }
      };
    });
      ...
      b.onclick = async function () {
        ...
        if (msg && msg.trim() !== "") {
          try {
            ...
            const res = await fetch(`${URL}/ideas/${id}/comment`, {
              method: "POST",
              headers: getRequestHeaders({ "Content-Type": "application/json" }),
              credentials: "include",
            ...
            if(res.ok) {
                ...
                alert("Failed to submit comment on server.");
            }
          } catch (err) {
              console.error("Comment dispatch error:", err);
          }
        }
      };
    });
    ...
    document.querySelectorAll(".del-btn").forEach((b) => {
      b.onclick = async function () {
        ...
        const id = this.getAttribute("data-id");
        try {
          const res = await fetch(`${URL}/ideas/${id}`, {
            method: "DELETE",
            headers: getRequestHeaders(),
            credentials: "include",
          });
  ...
  if (popupIdeaForm) {
      popupIdeaForm.onsubmit = async (e) => {
          ...
          const tagsInputEl = document.getElementById("idea-tags");
          ...
          try {
              ...
              if (res.ok) {
                  ...
                  const errData = await res.json().catch(() => ({}));
                  alert(errData.message || errData.error || "Failed to share idea.");
              }
          } catch (err) {
              console.error("Error sharing idea:", err);
              alert("Server error! Please try again later.");
